"""
Cost Model for Mixtral 8x7B MoE in FlexLLMGen.
...
Example Usages:
1. Find a policy:
python mixtral_cost_model.py --model mistralai/Mixtral-8x7B-Instruct-v0.1 --prompt-len 512 --gen-len 32 \
...
1. You need to fit the hardware constants for your device,
   see class MixtralCostModelConfig.
...
class MixtralCostModelConfig:
    ...
    l: int = 32  # num_hidden_layers
    h1: int = 4096  # hidden_size
    h2: int = 14336  # intermediate_size
    nh: int = 32  # num_attention_heads
    nkv: int = 8  # num_key_value_heads
    head_dim: int = 128  # h1 // nh
...
def get_mixtral_config(model_name):
    """Get Mixtral model configuration."""
    if model_name == "mistralai/Mixtral-8x7B-Instruct-v0.1":
        config = MixtralCostModelConfig()
        config.l = 32
        config.h1 = 4096
        config.h2 = 14336
        config.nh = 32
...
def solve_lp(config, bls, gbs, compress_w=False, verbose=1, debug=False, percent=None):
    ...
    # layer weight size for Mixtral (MoE structure)
    # Attention weights: q_proj, k_proj, v_proj, o_proj
    attn_weights = 4 * h1 * h1 + 2 * h1 * nkv * head_dim
    # MoE weights: gate + ALL experts (in large batch scenarios, all experts may be activated)
    # Gate: h1 * num_experts
    # Each expert: gate_proj + up_proj + down_proj = h1 * h2 + h1 * h2 + h2 * h1 = 3 * h1 * h2
    # ALL experts need to be transferred for large batch scenarios
    moe_weights = 2 * (h1 * num_experts + num_experts * 3 * h1 * h2) # fp 16
    # Layer norm weights
    norm_weights = 2 * h1
    
    wi = attn_weights + moe_weights + norm_weights
    if compress_w:
        wi = wi / 4
    ...
    # Attention computation: same as standard transformer
    attn_flops = 8 * s * h1 ** 2 + 4 * s * h1 * nkv * head_dim
    # MoE computation: gate + ALL experts (in large batch scenarios)
    # Gate computation: s * h1 * num_experts
    # Expert computation: ALL experts need to be computed as different tokens may route to different experts
    # Each expert: gate_proj + up_proj + down_proj = 2 * h1 * h2 + 2 * h2 * h1 = 4 * h1 * h2
    moe_flops = s * h1 * num_experts + num_experts * 4 * s * h1 * h2
    prob += compp == (1 / mm_flops_p) * bls * (attn_flops + moe_flops) \
    ...
    # GPU computation for generation
    gpu_attn_flops = 8 * h1 ** 2 + 4 * h1 * nkv * head_dim
    # MoE computation for generation: gate + ALL experts (in large batch scenarios)
    gpu_moe_flops = h1 * num_experts + num_experts * 4 * h1 * h2
    
    prob += compg == (1 / mm_flops_g) * bls * (gpu_attn_flops + gpu_moe_flops) \
    ...
    attn_inter = 8 * gbs * s * h1 + gbs * (2 * s * h1 + 2 * nh * s ** 2) + gbs * (2 * s * h1) + 4 * gbs * s * h1
    # MoE intermediate: gate logits + ALL expert outputs (in large batch scenarios)
    # Gate logits: gbs * s * num_experts
    # Expert intermediate activations: ALL experts need space for intermediate computations
    # Each expert: input (gbs * s * h1) + gate_proj output (gbs * s * h2) + up_proj output (gbs * s * h2) + final output (gbs * s * h1)
    moe_inter = gbs * s * num_experts + num_experts * (gbs * s * h1 + 2 * gbs * s * h2 + gbs * s * h1)
    
    prob += interp == attn_inter + moe_inter
    ...
    attn_inter_g = 8 * gbs * h1 + gbs * (2 * h1 + 2 * (s + n) * h1 + 2 * nh * (s + n)) * cg \
                   + gbs * (2 * (s + n) * h1 + 2 * h1) * cg + 4 * gbs * h1
    # MoE intermediate for generation: gate logits + ALL expert outputs
    # Gate logits: gbs * num_experts (for single token generation)
    # Expert intermediate activations: ALL experts need space
    # Each expert: input (gbs * h1) + gate_proj output (gbs * h2) + up_proj output (gbs * h2) + final output (gbs * h1)
    moe_inter_g = gbs * num_experts + num_experts * (gbs * h1 + 2 * gbs * h2 + gbs * h1)
    
    prob += interg == attn_inter_g + moe_inter_g
